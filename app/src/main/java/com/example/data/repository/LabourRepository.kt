...
class LabourRepository(private val labourDao: LabourDao) {
                ...
                if (snapshot != null) {
                    val list = snapshot.documents.mapNotNull { doc ->
                        try {
                            ...
                            val typeStr = doc.getString("attendanceType") ?: when(statusStr) {
                                "Present" -> "Full Day"
                                "Half Day" -> "Half Day"
                                "Absent" -> "Absent"
                                else -> "Full Day"
                            ...
                            Attendance(
                                ...
                                date = doc.getString("date") ?: "",
                                status = statusStr,
                                attendanceType = typeStr,
                                dailyRate = doc.getDouble("dailyRate") ?: 0.0,
                                earnedAmount = doc.getDouble("earnedAmount") ?: 0.0,
                                finalized = doc.getBoolean("finalized") ?: true,
                                finalizedAt = doc.getLong("finalizedAt") ?: (doc.getLong("createdAt") ?: System.currentTimeMillis()),
                                ...
                                createdAt = doc.getLong("createdAt") ?: System.currentTimeMillis(),
                                lastEditedAt = doc.getLong("lastEditedAt"),
                                lastEditedBy = doc.getString("lastEditedBy"),
                                autoGenerated = doc.getBoolean("autoGenerated") ?: false,
    ...
    suspend fun insertPayment(payment: Payment): Long {
        ...
        firestore.collection("payments")
            ...
            .awaitTask()
        // Also cache locally
        labourDao.insertPayment(updated)
        return id.toLong()
    }

    suspend fun deletePayment(payment: Payment) {
        firestore.collection("payments")
            .document("${payment.userId}_${payment.id}")
            .delete()
            .awaitTask()
    ...
    // Site Expenses ops
    fun getAllSiteExpenses(userId: String): Flow<List<SiteExpense>> = callbackFlow {
        val listener = firestore.collection("site_expenses")
            .whereEqualTo("userId", userId)
            .addSnapshotListener { snapshot, error ->
                if (error != null) {
                    close(error)
                ...
                if (snapshot != null) {
                    val list = snapshot.documents.mapNotNull { doc ->
                        try {
                            SiteExpense(
                                ...
                                expenseDate = doc.getString("expenseDate") ?: "",
                                createdAt = doc.getLong("createdAt") ?: System.currentTimeMillis(),
                                updatedAt = doc.getLong("updatedAt") ?: System.currentTimeMillis()
                            )
                        } catch (e: Exception) {
                    ...
                    }.sortedWith(compareByDescending<SiteExpense> { it.expenseDate }.thenByDescending { it.id })
                    trySend(list)
                }
            }
        awaitClose { listener.remove() }
    ...
    suspend fun insertSiteExpense(expense: SiteExpense): Long {
        val id = if (expense.id == 0) (System.currentTimeMillis() % 1000000000).toInt() + (1..100000).random() else expense.id
        val updated = expense.copy(id = id, updatedAt = System.currentTimeMillis())
        try {
            firestore.collection("site_expenses")
                .document("${updated.userId}_${updated.id}")
                .set(updated)
                .awaitTask()
        } catch (e: Exception) {
            // Force write through local DAO cache anyway
        }
        labourDao.insertSiteExpense(updated)
        return id.toLong()
    }
    ...
    suspend fun updateSiteExpense(expense: SiteExpense) {
        ...
        try {
            ...
            // Ignore if offline or error occurs
        }
        labourDao.updateSiteExpense(updated)
    }

    suspend fun deleteSiteExpense(expense: SiteExpense) {
        try {
            firestore.collection("site_expenses")
        ...
        labourDao.deleteSiteExpenseById(expense.id, expense.userId)
    }

    fun getAllAttendanceAuditLogs(userId: String): Flow<List<AttendanceAuditLog>> = callbackFlow {
        val listener = firestore.collection("attendance_audit_logs")
            .whereEqualTo("userId", userId)
            .addSnapshotListener { snapshot, error ->
                if (error != null) {
                    close(error)
                    ...
                    val list = snapshot.documents.mapNotNull { doc ->
                        try {
                            AttendanceAuditLog(
                                ...
                                editedBy = doc.getString("editedBy") ?: "",
                                editedAt = doc.getLong("editedAt") ?: System.currentTimeMillis(),
    ...
    suspend fun insertAttendanceAuditLog(log: AttendanceAuditLog): Long {
        val id = if (log.id == 0) (System.currentTimeMillis() % 1000000000).toInt() + (1..100000).random() else log.id
        val updated = log.copy(id = id)
        firestore.collection("attendance_audit_logs")
            .document("${updated.userId}_${updated.id}")
            .set(updated)
            .awaitTask()
        labourDao.insertAttendanceAuditLog(updated)
        return id.toLong()
    }
